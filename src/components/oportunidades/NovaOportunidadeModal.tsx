...
export function NovaOportunidadeModal({ open, onOpenChange, onSave }: NovaOportunidadeModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    gestao: "", operacao: "", observacoes: "",
  });

  const resetForm = () => {
    setStep(1);
    setForm({ nomeFantasia: "", razaoSocial: "", cnpj: "", segmento: "", cidade: "", estado: "", email: "", telefone: "", gestao: "", operacao: "", observacoes: "" });
  };
  ...
  const step2Valid = form.gestao && form.operacao;

  const availableOps = form.gestao ? gestaoOperacoes[Number(form.gestao)] || [] : [];

  const handleGestaoChange = (v: string) => {
    setForm({ ...form, gestao: v, operacao: "" });
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Passo {step} de 2 — {step === 1 ? "Dados do Cliente" : "Gestão e Operação"}
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 2 && (
          <div className="space-y-4 pt-2">
            <div>
              <Label>Gestão *</Label>
              <Select value={form.gestao} onValueChange={handleGestaoChange}>
                <SelectTrigger><SelectValue placeholder="Selecione a gestão" /></SelectTrigger>
                <SelectContent className="bg-card z-50">
                  <SelectItem value="1">Gestão 1</SelectItem>
                  <SelectItem value="2">Gestão 2</SelectItem>
                  <SelectItem value="3">Gestão 3</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label>Operação *</Label>
              <Select value={form.operacao} onValueChange={(v) => setForm({ ...form, operacao: v })} disabled={!form.gestao}>
                <SelectTrigger><SelectValue placeholder={form.gestao ? "Selecione a operação" : "Selecione a gestão primeiro"} /></SelectTrigger>
                <SelectContent className="bg-card z-50">
