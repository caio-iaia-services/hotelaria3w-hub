...
function MetricCard({ icon, titulo, valor, cor }: MetricCardProps) {
  ...
  return (
    <div className="bg-card border rounded-lg p-4">
      <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center mb-3", cores[cor])}>{icon}</div>
...
function FunilVertical({ cards, cardSelecionado, onSelecionar }: FunilProps) {
  return (
    <div className="p-4 space-y-3">
      ...
      {cards.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground text-sm">Nenhum lead ativo</div>
      ) : (
        cards.map((card) => (
          <button
            key={card.id}
            ...
            <div className="flex items-start justify-between mb-2 gap-2">
              ...
              <Badge
                variant={
                  card.estagio === "lead"
                    ? "secondary"
                    : card.estagio === "contato"
                      ? "default"
                      : card.estagio === "proposta"
                        ? "outline"
                        : card.estagio === "negociacao"
                          ? "destructive"
                          : "default"
                }
...
export default function AcoesComerciais() {
  ...
  function selecionarOperacao(operacao: string, listaFornecedores?: FornecedorLocal[]) {
    ...
    const fornecedor = lista.find((f) => f.nome_fantasia.toUpperCase() === operacao.toUpperCase());
  ...
  async function gerarOrcamento() {
    ...
    try {
      ...
      console.log("💰 VALORES CALCULADOS:", {
        ...
        valorFrete,
        total,
        itensDetails: itensOrcamento.map((i) => ({
          desc: i.descricao,
          qty: i.quantidade,
      ...
      async function inserirOrcamentoComFallback(payloadBase: Record<string, unknown>) {
        ...
        for (let tentativa = 0; tentativa < 20; tentativa += 1) {
          ...
          const colunaAusente =
            error.code === "PGRST204" ? error.message.match(/Could not find the '([^']+)' column/)?.[1] : null;
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-[#dbdbdb] -m-6">
      ...
      <div className="grid grid-cols-6 gap-4 px-6 py-4 border-b bg-muted/30">
        ...
        <MetricCard
          ...
          valor={metricas.orcamentosEnviados}
          cor="green"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          titulo="Tempo Médio"
          valor={`${metricas.tempoMedio} dias`}
          cor="gray"
        />
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 border-r bg-muted/20 overflow-y-auto">
          {loading ? (
            ...
            <FunilVertical cards={cards} cardSelecionado={cardSelecionado} onSelecionar={setCardSelecionado} />
          )}
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {cardSelecionado ? (
            <AreaTrabalho card={cardSelecionado} documentos={documentos} onAcao={executarAcao} />
          ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDadosOrcamento((prev) => ({ ...prev, validade_dias: parseInt(e.target.value) || 30 }))
                  }
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">Valor: {formatCurrency(calcularValorImpostos())}</p>
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDadosOrcamento((prev) => ({ ...prev, desconto: parseFloat(e.target.value) || 0 }))
                  }
                />
                <p className="text-xs text-muted-foreground mt-1">Valor: -{formatCurrency(calcularValorDesconto())}</p>
              </div>
              <div>
            ...
            <div className="border-2 border-dashed border-[#c4942c]/40 rounded-lg p-6 bg-[#c4942c]/5">
              ...
              {imagemMarketing ? (
                <div className="relative">
                  <img src={imagemMarketing.preview} alt="Campanha Marketing" className="w-full rounded-lg shadow-md" />
                  ...
                  <div className="absolute top-2 right-2 flex gap-2">
                    ...
                    <Button type="button" variant="destructive" size="sm" onClick={removerImagemMarketing}>
                ...
                <div className="text-center py-8 text-[#c4942c]">
                  ...
                  <p className="text-xs text-[#c4942c]/60 mt-1">JPG, PNG • Máx 5MB • Recomendado: 1200x400px</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-gray-300 rounded-lg p-6 shadow-lg">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-[#c4942c]">+{formatCurrency(calcularValorImpostos())}</span>
                </div>

                {/* DESCONTO */}
                {dadosOrcamento.desconto > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-green-700">Desconto ({dadosOrcamento.desconto}%):</span>
                    <span className="font-semibold text-green-700">-{formatCurrency(calcularValorDesconto())}</span>
                  </div>
                )}
                ...
                <div className="flex justify-between items-center pb-3 border-b-2 border-gray-400">
                  ...
                  <span className="font-semibold text-blue-700">+{formatCurrency(dadosOrcamento.frete || 0)}</span>
                </div>
